...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:AZIZAE\n'
            + 'ORG:Creator aebot;\n'
            + 'TEL;type=CELL;type=VOICE;waid=994405327962:+994 40 532 79 62\n'
            + 'END:VCARD'
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const AzizAE = ["994405327962@s.whatsapp.net"]
		...
		case 'daftar':
         ...
         case 'lirik':
                ...
                if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('994405327962@s.whatsapp.net', options, text, {quoted: mek})
...
//akhir kreator
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ Tag By *${pushname2}* ✪══`+ teks +'╚═══〘 AE BOT 〙══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 ae BOT 〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
          ...
          case 'snack':
			if (isBanned) return reply(mess.only.benned)    
               ...
               case 'ytmp4':
					...
					anu = await fetchJson(`https://api.zeks.xyz/api/ytmp4?url=${args[0]}&apikey=apivinz`)
					if (anu.error) return reply(anu.error)
